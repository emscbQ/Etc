...
if __name__ == "__main__":
    ...
    log_list = list()
    log_list.append(get_now() + "프로그램 시작")

    try:
        # NOTE Get excel
        print("정보통에서 추출한 엑셀 파일을 선택해주세요.")
        log_list.append(get_now() + "엑셀 읽기 시작")
        excel_data = open_selected_excel_to_list()
        log_list.append(get_now() + "엑셀 읽기 완료")
        ...
        for row in excel_data[1:]:
            ...
            try:
                ...
                log_list.append(get_now() + f"상품명이 패턴에 맞지 않습니다. ({row[product_name_col]})")
                continue
        ...
        for order in order_name_dict:
            ...
            log_list.append(get_now() + f"{order} 조회 시작")
            response = requests.post(url=urls.get("post_list_warehouse_out"), headers=header, data=json.dumps(body))
            if response.status_code == 200:
                log_list.append(get_now() + f"{order} 조회 성공")
                rsp = response.json()
                for warehouse_out in rsp["result"]["warehouseOuts"]:
                    if warehouse_out == "":
                        print(f"주문 번호로 조회된 출고 지연 등록 가능한 상품이 없습니다. ({order})")
                        log_list.append(get_now() + f"주문 번호로 조회된 출고 지연 등록 가능한 상품이 없습니다. ({order})")
                        break
                    log_list.append(get_now() + f"{len(rsp['result']['warehouseOuts'])}개 조회 완료")
                    if warehouse_out.get("warehouseOut").get("itemNm") in order_name_dict[order]:
                        # 출고지연처리
                        delay_body = dict(
                            requestNoSellRequestRegist=dict(
                                shppNo=warehouse_out.get("warehouseOut").get("shppNo"),  # 배송번호
                                scEvnt="I",  # 등록(I)/삭제(D)
                                shppSeq=warehouse_out.get("warehouseOut").get("shppSeq"),  # 배송순번
                                shortgRsnCd="09",  # 등록/삭제 구분 / 09: 결품, 품절(재고부족)
                                shortgProcDtlc="품절",  # 판매불가사유내용
                                itemId=warehouse_out.get("warehouseOut").get("itemId"),  # 상품번호
                            )
                        )
                        ...
                        if delay_response.status_code == 200:
                            if delay_response.json()["result"]["resultMessage"] == "SUCCESS":
                                print("출고 지연 등록 성공")
                                log_list.append(
                                    get_now() + f"{warehouse_out.get('warehouseOut').get('shppNo')} / {warehouse_out.get('warehouseOut').get('itemId')} 출고 지연 등록 완료")
                            else:
                                print("출고 지연 등록 실패")
                                log_list.append(
                                    get_now() + f"{warehouse_out.get('warehouseOut').get('shppNo')} / {warehouse_out.get('warehouseOut').get('itemId')} 출고 지연 등록 실패")
                        else:
                            print("출고 지연 등록 실패")
                            log_list.append(
                                get_now() + f"{warehouse_out.get('warehouseOut').get('shppNo')} / {warehouse_out.get('warehouseOut').get('itemId')} 출고 지연 등록 실패")
                ...
                log_list.append(get_now() + f"{order} 조회 실패")
